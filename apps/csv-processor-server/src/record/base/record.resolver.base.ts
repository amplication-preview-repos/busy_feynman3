...
export class RecordResolverBase {
  ...
  async createRecord(@graphql.Args() args: CreateRecordArgs): Promise<Record> {
    return await this.service.createRecord({
      ...
      data: {
        ...
        csvFile: args.data.csvFile
          ? {
              connect: args.data.csvFile,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => CsvFile, {
    nullable: true,
    name: "csvFile",
  })
  async getCsvFile(@graphql.Parent() parent: Record): Promise<CsvFile | null> {
    const result = await this.service.getCsvFile(parent.id);
